...
// 3. 单指南无冲突 → ""
ok(
  buildVersionConflictHint([{ file_path: "medical-raw-txt/1.原发性肝癌诊疗指南（2026版）.txt" }], GM) === "",
  "正常现行指南 → 空提示",
);

// 4. 命中已废止指南 → 含「已废止」
{
  const h = buildVersionConflictHint([{ file_path: "medical-raw-txt/2.原发性肝癌诊疗指南（2019版）.txt" }], GM);
...
{
  const h = buildVersionConflictHint([{ file_path: "medical-raw-txt/3.肺炎诊疗指南（2023版）.txt" }], GM);
...
// 6. 多指南（一正常一废止）→ 仅标废止的
{
  const h = buildVersionConflictHint(
    [
      { file_path: "medical-raw-txt/1.原发性肝癌诊疗指南（2026版）.txt" },
      { file_path: "medical-raw-txt/2.原发性肝癌诊疗指南（2019版）.txt" },
    ],
    GM,
  );
