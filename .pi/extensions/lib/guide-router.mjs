...
const PHRASE_ALIASES = [
  ...
  ["恶性淋巴瘤", "淋巴瘤"],
  ["子宫内膜恶性肿瘤", "子宫内膜癌"],
  // —— 症状/口语 → 规范病种（急诊与同义表述归一，修复路由空结果/错配）——
  ["怀孕", "妊娠"],
  ["孕妇", "妊娠"],
  ["一边脸歪", "脑卒中"],
  ["口角歪斜", "脑卒中"],
  ...
  ["感冒药", "普通感冒"],
  ["多种感冒药", "普通感冒"],
  // —— 病种缩写/口语 → 规范病种（A 轨补录配套，修复路由漂移）——
  ["慢阻肺", "慢性阻塞性肺疾病"], // Q26：COPD 缩写归一
  ["COPD", "慢性阻塞性肺疾病"],
];
...
export function routeGuides(query, opts = {}) {
  ...
  for (const [title, info] of Object.entries(guideMap)) {
    ...
    // 妊娠/围产亚人群 WS 标准：查询未涉妊娠语境时相对通用病种指南降权，
    // 避免「WST 妊娠期糖尿病」等亚人群标准在非妊娠糖尿病问法中挤占通用指南 top3
    // （如 Q36 二甲双胍增强 CT：妊娠糖尿病体重/膳食标准与通用糖尿病指南平局却排序靠前）。
    // 查询含妊娠/孕妇/围产时不降权，保证妊娠相关问法正常命中。
    const isPregnancyWs =
      /^(ws|gbz)/i.test(title) &&
      /妊娠|孕妇|围产|妇女/.test(title + (info.disease || ""));
    const qMentionsPregnancy = /妊娠|孕妇|围产/.test(qNorm);
    if (isPregnancyWs && !qMentionsPregnancy && matchedKw.length > 0) {
      score *= 0.6;
      reasons.push("WS妊娠亚人群非语境降权");
    }
