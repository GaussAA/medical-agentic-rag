...
```bash
# 前台（终端关闭即停）
scripts/start/start.sh webui

# 后台（脱离终端，写 PID，可长期运行）
scripts/start/start.sh webui -d

# 停止
scripts/start/start.sh stop webui
...
```bash
scripts/start/start.sh api                    # 前台（终端关闭即停）
scripts/start/start.sh api -d                # 后台守护（写 .pi/api.pid，日志 .pi/logs/api.log）
scripts/start/start.sh stop api              # 停止
API_PORT=9090 API_TOKEN=xxxx scripts/start/start.sh api -d   # 自定义端口 + 开启 Bearer 鉴权
```
